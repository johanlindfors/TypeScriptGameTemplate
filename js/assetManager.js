        ...
        Object.keys(that.assets).forEach(function (asset, i) {
            switch(that.assets[asset].fileType) {
                case AssetType.image: {
                    ...
                    that.assets[asset].object.addEventListener("load", function (okEvent) {
                        that.loadCompleted(okEvent, i);
                    }, false);
                    that.assets[asset].object.addEventListener("error", function (error) {
                        that.loadFailed(error, i);
                    }, false);
                ...
                case AssetType.audio: {
                    ...
                    that.assets[asset].object.addEventListener("canplaythrough", function (okEvent) {
                        that.soundLoadCompleted(okEvent, i);
                    }, false);
                    that.assets[asset].object.addEventListener("error", function (error) {
                        that.loadFailed(error, i);
                    }, false);
                    if(!!that.assets[asset].object.canPlayType("audio/mpeg") && !!that.assets[asset].object.canPlayType("audio/ogg") && !!that.assets[asset].object.canPlayType("audio/wav")) {
                        that.assets[asset].object = null;
                    }
